...
def convert_mctest_to_csv(X_df, y_df):
    ...
    def replacer(x): return x.replace('multiple: ', '').replace('one: ', '')
    questions = np.array([replacer(q) for q in questions])

    answers = X_df[[4, 5, 6, 7, 9, 10, 11, 12,
                    14, 15, 16, 17, 19, 20, 21, 22]].copy()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Structure MCTest raw data to csv.')
    parser.add_argument('--input_dir', type=str,
                        help='path to raw directory containing .tsv and .ans files')
    ...
    X_dev_df = pd.read_csv(input_dir + 'mc500.dev.tsv', sep='\t', header=None)
    y_dev_df = pd.read_csv(input_dir + 'mc500.dev.ans', sep='\t', header=None)

    X_train_df = pd.read_csv(
        input_dir + 'mc500.train.tsv', sep='\t', header=None)
    y_train_df = pd.read_csv(
        input_dir + 'mc500.train.ans', sep='\t', header=None)

    X_test_df = pd.read_csv(
        input_dir + 'mc500.test.tsv', sep='\t', header=None)
    y_test_df = pd.read_csv(
        input_dir + 'mc500.test.ans', sep='\t', header=None)
    ...
    train_output.to_csv(output_dir + 'mctest_train.csv')

    test_output = convert_mctest_to_csv(X_test_df, y_test_df)
    test_output.to_csv(output_dir + 'mctest_test.csv')
